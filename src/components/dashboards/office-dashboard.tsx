    ...
    <div>
      ...
      {s && s.draft > 0 && (
        <Link href="/marks" className="mb-6 flex items-center gap-3 rounded-md border border-watch/40 bg-watch-soft px-4 py-3 transition-colors hover:border-watch">
          ...
          <div className="text-[14px] text-ink-900"><span className="font-semibold">{s.draft}</span> {t("marks.draft").toLowerCase()} — <span className="text-gold-700 underline">{t("marks.release").toLowerCase()}</span></div>
        </Link>
      )}
